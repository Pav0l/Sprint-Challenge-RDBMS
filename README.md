...
`RDBMS` = Relational DataBase Management Systems  - a software to manage relational databases.
`SQL` = Structured Query Language - is the standard language to manage databases and data within them, it's a way to interact with `RDBMS`.


1. Why do tables need a `primary key`?

Tables need primary keys to uniquely identify each record in the table.
...
Each row in table one can refference many rows in table two and vice versa. 
